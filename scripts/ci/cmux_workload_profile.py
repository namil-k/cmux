...
def source_identity(expected_commit: str | None, expected_tree: str | None) -> dict[str, str]:
    ...
    tracked_dirty = subprocess.run(
        ["/usr/bin/git", "diff", "--quiet", "HEAD", "--"], cwd=ROOT, check=False
    ).returncode
...
def prepare_state_root(
    ...
    if raw is None:
        ...
        if resolved != path or not path.is_dir() or path.is_symlink():
            raise ProfileError("state root must be one canonical plain directory")
...
def toolchain_summary(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if profile["platform"]["os"] == "macos":
        for key, argv in (
            ...
            value = command_text(argv)
            if value:
                observations[key] = value
    else:
        value = command_text(["/bin/bash", "--version"])
        if value:
...
def run_profile(args: argparse.Namespace) -> int:
    ...
    toolchain = toolchain_summary(profile)
